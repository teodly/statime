...
use statime::{
    ...
    overlay_clock::{CallbackExporter, OverlayClock},
    port::{
...
async fn actual_main() {
    ...
    let allowed_interfaces = config.ports.iter().map(|port|port.interface).collect::<Vec<_>>();
...
fn get_clock_id(allowed_names: &[InterfaceName]) -> Option<[u8; 8]> {
    let candidates = interfaces()
        ...
        .filter_map(|(name, data)| {
            if allowed_names.contains(&name) { data.mac() } else { None }
        });
